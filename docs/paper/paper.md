...
Kernels propose. They do not accept. A kernel returns (y, log q(y|x), log q(x|y)). The engine computes
    ...
    α = min(1, exp(Δlog p + log q_rev − log q_fwd)).
...
- **Uniform flip.** Propose a uniform random bitstring. Symmetric. The weakest baseline. Included so we can say we beat it, which we do, which does not matter.
...
The learned surrogate is not the alibi. Sampled Spearman is 0.998 at every p = 20 and p = 27 cell. Analytic collapses (0.37 down to −0.11). Landscape quality did not enter accept/reject: ADS PIP error stays at Monte Carlo.

## 5. Noise as a proposal temperature

The folklore: depolarizing noise heats the proposal, a hotter proposal mixes, therefore scheduled noise is free tempering. The construction that makes that legal is a *fixed* ladder. Each rung is a quench at a locked λ. A random-scan mixture of symmetric kernels is symmetric. Picking the rung from the current state is a different kernel and needs log-q or diminishing adaptation. We did not do that.
...
We did not go to IBM. We ran the same audit on Aer amplitude damping, diabetes p = 10, four chains of 8000 steps, γ ∈ {0.01, 0.05, 0.1}, against the enumerated posterior. Ideal quench (no extra channel) and add-delete-swap sit on the same floor.
...
Vuffray, M., et al. (2022). High-quality thermal Gibbs sampling with quantum annealing hardware. *Phys. Rev. Applied* 17, 044046. arXiv:2109.01690.
